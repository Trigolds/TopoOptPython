...
class MGPCG_Jacobi_V:
    def __init__(self):
        ...
        self.r = [ti.var(dt=lp) for _ in range(self.n_mg_levels)]  # residual
        self.e = [ti.var(dt=lp) for _ in range(self.n_mg_levels)]  # M^-1 self.r
        self.x = ti.var(dt=lp)  # solution
        self.x_ref = ti.var(dt=lp)  # solution
        self.res = ti.var(dt=lp)  # solution
        ...
        indicies = ti.ij
        self.grid = ti.root.dense(indicies, self.N_tot).place(self.x, self.x_ref, self.b, self.p, self.Ap, self.res)
        self.grid = ti.root.dense(indicies, self.N_tot).place(self.r[0], self.e[0])
        for l in range(self.n_mg_levels - 1):
            self.grid = ti.root.dense(indicies,self.N_tot // 2**(l+1) + 1).place(self.r[l+1], self.e[l+1])
    ...
    def update_p(self):
        for I in ti.grouped(self.p):
            self.p[I] = self.e[0][I] + self.beta[None] * self.p[I]
    ...
    def MG(self, u, b):
        ...
        if(u.shape[0]>2**(self.n_mg_levels-3)+1):
            r = self.residual(u, b)
            rc = self.restrit(r)
