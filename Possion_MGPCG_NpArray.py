...
class MGPCG_Jacobi_V:
    def __init__(self):
        ...
        self.e = np.arange(self.N_tot*self.N_tot, dtype=float).reshape(self.N_tot, self.N_tot)
        self.x = np.arange(self.N_tot*self.N_tot, dtype=float).reshape(self.N_tot, self.N_tot)  
        self.x_ref = np.arange(self.N_tot*self.N_tot, dtype=float).reshape(self.N_tot, self.N_tot) 
        self.res = np.arange(self.N_tot*self.N_tot, dtype=float).reshape(self.N_tot, self.N_tot)  
        self.b = np.arange(self.N_tot*self.N_tot, dtype=float).reshape(self.N_tot, self.N_tot)  
        self.p = np.arange(self.N_tot*self.N_tot, dtype=float).reshape(self.N_tot, self.N_tot)  # conjugate gradient
        self.Ap = np.arange(self.N_tot*self.N_tot, dtype=float).reshape(self.N_tot, self.N_tot)  # matrix-vector product
    ...
    def reduce(self, p, q):
        ...
        for i in range(self.N_tot):
            for j in range(self.N_tot):
                self.sum += p[i,j] * q[i,j]

    
    def update_x(self):
        for i in range(self.N_tot):
            for j in range(self.N_tot):
                self.x[i,j] += self.alpha * self.p[i,j]

    
    def update_r(self):
        for i in range(self.N_tot):
            for j in range(self.N_tot):
                self.res[i,j] -= self.alpha * self.Ap[i,j]

    
    def update_p(self):
        for i in range(self.N_tot):
            for j in range(self.N_tot):
                self.p[i,j] = self.e[i,j] + self.beta * self.p[i,j]
